...
import { API_URL, API_KEY, IMAGE_URL } from './api_config';
import qs from 'qs';
...
export const getMovies = async (filter) => {
    ...
    const response = await movies.get(`${url}/movie/${filter}`, { params });
...
export const getTvShows = async (filter) => {
    ...
    console.log('PARAMETERS: ', params);
    console.log('FILTER: ', filter);
    const response = await tvShows.get(`${url}/tv/${filter}`, { params });
    console.log('RESPONSE:', response.data.results);
